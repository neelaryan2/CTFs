...
<?php
	...
	define("INSTANCEID",    "oc70x9zddm1h");
	define("SECRET",        "W7gQDN1YyCz/i0JaGUGAGuy0lQwwCtdozIaGHAY362Ol2A73");
	...
	function main($argv) {
		...
		$file = "mcap.7z";
		$filename = "data/JFree/files/${file}";
		$target = "output_${file}";
		$filekeyname = "data/JFree/files_encryption/keys/files/${file}/OC_DEFAULT_MODULE/fileKey";
		$sharekeyname = "data/JFree/files_encryption/keys/files/${file}/OC_DEFAULT_MODULE/master_97316165.shareKey";
		...
		foreach ($privatekeys as $key => $value) {
			...
			if ((false !== $filekey) && (false !== $sharekey)) {
				if (openssl_open($filekey, $tmpkey, $sharekey, $privatekeys[$key], "rc4")) {
					$secretkey = $tmpkey;
					decryptFile($filename, $secretkey, $target);
					break;
				} else {
					debug("secretkey could not be decrypted");
				}
